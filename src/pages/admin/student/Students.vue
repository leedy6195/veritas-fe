...
const updateMember = () => {
  ...
  axios.put(`/api/students/${editedMember.value.id}`, editedMember.value)
      .then((response) => {
...
const deleteSelectedMembers = async () => {
  if (confirm('선택한 회원을 삭제하시겠습니까?')) {
    const deletePromises = selectedMembers.value.map(memberId => axios.delete(`/api/students/${memberId}`));
...
const addMember = () => {
  ...
  axios.post('/api/students', newMember.value)
      .then((response) => {
        if (response.data.header.success) {
          location.reload()
        } else {
