...
const fetchRooms = () => {
  axios.get('/api/lecturerooms').then((response) => {
...
const updateLectureRoom = () => {
  ...
  axios.put(`/api/lecturerooms/${editedLectureRoomId.value}`, {
...
const deleteLectureRoom = () => {
  if (confirm('정말 삭제하시겠습니까?')) {
    axios.delete(`/api/lecturerooms/${editedLectureRoomId.value}`).then((response) => {
      if (response.data.header.success) {
        fetchRooms()
        editLectureRoomDialog.value = false
      }
